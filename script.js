...
function getScrollDistance() {
  ...
  return clamp(-section.getBoundingClientRect().top, 0, maxScroll);
}
...
function updateSightSlider() {
  // Guard: nothing to do if the slider isn't ready or cards haven't
  // been laid out yet (e.g. images still loading, width would read 0).
...
// Nav / "open old town notes" scroll targets: these jump the page to a
// specific point along the scroll-driven story instead of relying on the
// browser's default anchor jump, which doesn't work correctly here since
// the story content is positioned with transforms inside a sticky stage.
function setupScrollTargets() {
  const links = document.querySelectorAll("[data-scroll-target]");
  links.forEach((link) => {
    link.addEventListener("click", (e) => {
      const target = Number(link.dataset.scrollTarget);
      if (!section || !Number.isFinite(target)) return;
      e.preventDefault();
      const sectionTop = section.getBoundingClientRect().top + window.scrollY;
      window.scrollTo({
        top: sectionTop + target,
...
// Sight-pin thumbnails load lazily; once everything has settled, re-measure
// card widths in case late-loading images changed layout.
